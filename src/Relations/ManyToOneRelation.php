    ...
    public function createSelectAndAutoLinks($id, $record)
    {
        ...
        if ($this->hasFlag(self::AF_RELATION_AUTOLINK)) { // auto edit/view link
            ...
            if ($this->m_destInstance->allowed('edit') && !$this->m_destInstance->hasFlag(Node::NF_NO_EDIT) && $record[$this->fieldName()] != null) {
                ...
                $links[] = '<span id="'.$id."_edit\" style=\"\"><a href='javascript:atkSubmit(mto_parse(\"".Tools::atkurlencode($editUrl).'", document.entryform.'.$id.".value), true)' class=\"atkmanytoonerelation atkmanytoonerelation-link\">".$this->getOwnerInstance()->getLanguage()->trans('edit').'</a></span>';
            }
        }
        ...
        if ($this->hasFlag(self::AF_RELATION_AUTOLINK)) { // auto edit/view link
            ...
            if (!$this->m_destInstance->hasFlag(Node::NF_NO_EDIT) && $this->m_destInstance->allowed('edit')) {
                $editlink = $sm->sessionUrl(Tools::dispatch_url($this->getAutoLinkDestination(), 'edit', array('atkselector' => 'REPLACEME')),
                    SessionManager::SESSION_NESTED);
                $autolink['edit'] = "<a href='javascript:atkSubmit(mto_parse(\"".Tools::atkurlencode($editlink).'", document.entryform.'.$id.".value),true)' class='atkmanytoonerelation atkmanytoonerelation-link'>".$this->getOwnerInstance()->getLanguage()->trans('edit').'</a>';
            }
    ...
    public function search($record, $extended = false, $fieldprefix = '', DataGrid $grid = null)
    {
        ...
        if (!$this->hasFlag(self::AF_LARGE) && !$useautocompletion) {
            ...
            if ($useautocompletion) {
                $noneLabel = $this->getOwnerInstance()->getLanguage()->trans('search_all');
